	...
	if($checkExpiration == true)
		{
			if(isset($check_login['bad_logins']) && isset($check_login['max_attempts_allowed']) && ($check_login['bad_logins'] >= $check_login['max_attempts_allowed']))
			{
					...
					$externalMsg = $langGlobal['ip_ban_temp'];
			}
			else
			{
				...
				if ((empty($username_dec)) || (empty($password_dec))) {
					$db->query("INSERT INTO $db->table_accountlogs (message_fail, user_ip) VALUES ('Es wurden nicht alle erforderlichen Informationen eingegeben', ('" . $client_ip . "'))");
					$externalMsg = $langGlobal['data_missed'].'<br />';
					$errorStatus = true;
					...
					if(mysqli_num_rows($getCrypt) == 1)
					{
						...
						$user_counter        = $db->query("SELECT id FROM $db->table_sessions WHERE active= '1'");
						$userLimitation 	 = $main->serverConfig("user_capacity_system");
						
						// DB DATA
						$user_db = $db->query("SELECT username FROM $db->table_accounts WHERE username = ('" . $username_validated . "')");
						$password_db = $db->query("SELECT pass_hash FROM $db->table_accounts WHERE pass_hash = ('" . $pass_hash_final . "') AND username = ('" . $username_validated . "') ");
						$login_ip_update = ("UPDATE $db->table_accounts SET last_login_ip= ('" . $client_ip . "') WHERE username= ('" . $username_validated . "') AND pass_hash=('" . $pass_hash_final . "')");
						$login_sql = $db->query("SELECT username, pass_hash FROM $db->table_accounts WHERE username= ('" . $username_validated . "') AND pass_hash= ('" . $pass_hash_final . "')");
						$ban_check = $db->query("SELECT id, ban_type, ban_duration, banned_by, banned_at FROM $db->table_ban WHERE id= ( SELECT id from $db->table_accounts WHERE username=('" . $username_validated . "') AND pass_hash= ('" . $pass_hash_final . "') )");
						...
						if ($userLimitation >= 1) {
							if ($max_users <= mysqli_num_rows($user_counter)) {
								$db->query("INSERT INTO $db->table_accountlogs (message_fail, user_ip) VALUES ('Server Kapazität wurde überschritten. User wurde nicht eingeloggt.', ('" . $client_ip . "'))");
								$externalMsg = $langGlobal['max_users_reached_string_first'] .  $max_users  . $langGlobal['max_users_reached_string_sec'].'<br />';
								$errorStatus = true;
						...
						if (preg_match($user_chars, $username_validated)) {
							$db->query("INSERT INTO $db->table_accountlogs (message_fail, user_ip) VALUES ('Username enthält ungueltige Zeichen', ('" . $client_ip . "'))");
								$externalMsg = $langGlobal['username_illegal_chars'].'<br />';
								$errorStatus = true;
						} //preg_match( $user_chars, $username_validated )
						
						// IF ACCOUNT DONT EXISTS
						if (mysqli_num_rows($user_db) == 0 && mysqli_num_rows($password_db) == 0) {
							$db->query("INSERT INTO $db->table_accountlogs (message_fail, user_ip) VALUES ('Ungültiger Username und Passwort!', ('" . $client_ip . "'))");
								$externalMsg = $langGlobal['invalid_pass_or_username'].'<br />';
								$errorStatus = true;
						...
						if (mysqli_num_rows($user_db) == 1 and mysqli_num_rows($password_db) == 0) {
							$db->query("INSERT INTO $db->table_accountlogs (message_fail, account_id, sid, user_ip) VALUES ('Ungültiges Passwort', (SELECT id FROM $db->table_accounts WHERE username= ('" . $username_validated . "')), ('" . $_SESSION['ID'] . "'), ('" . $client_ip . "'))");
								sleep(2);
								$externalMsg = $langGlobal['invalid_pass_or_username'].'<br />';
								$errorStatus = true;
						...
						while ($active = mysqli_fetch_object($active_result)) {
							...
							if ($accepted == 0) {
								$db->query("INSERT INTO $db->table_protection_logs (message, account_id, user_ip) VALUES ('Versuchter User Login während Account mit Verifizierungssperre versehen ist!', (SELECT id FROM $db->table_accounts WHERE username= ('" . $username_validated . "')), ('" . $client_ip . "'))");
								$externalMsg = $langGlobal['account_not_validated'].'<br />';
								$errorStatus = true;
							} //$accepted == 0
						...
						switch($ban_type) {
							...
							if (date('Y-m-d', $realtime_duration) == date('Y-m-d')) {
								...
								$realtime_duration = strftime($langGlobal['account_ban_temp_time_first']."%A, %d %B %Y %H:%M", $realtime_duration);
							}
									
								if($time_remaining >= 1) $string = $langGlobal['account_ban_temp_first'] . $realtime_duration . $langGlobal['account_ban_temp_second'];
								break;
							...
							$db->query("INSERT INTO $db->table_accountlogs (message_fail, account_id, sid, user_ip) VALUES ('Loginversuch während Accountsperre', (SELECT id FROM $db->table_accounts WHERE username= ('" . $username_validated . "')), ('" . $_SESSION['ID'] . "'), ('" . $client_ip . "'))");
							$externalMsg = $string;
							$errorStatus = true;
						}
						...
						if (mysqli_num_rows($login_sql) == 1 && ($accepted) == 1 && (mysqli_num_rows($ban_check) == 0 || $time_remaining <= 0))
						{
							...
							$db->query("UPDATE $db->table_accounts SET logged_in= '1', sid= ('" . $_SESSION['ID'] . "'), last_login= NOW(), last_login_ip= ('" . $client_ip . "'), persistent_session_status= ('" . $persistentSess . "') WHERE username= ('" . $username_validated . "') AND pass_hash=('" . $pass_hash_final . "')");
							...
							if (mysqli_num_rows($set_user_session_check) != 0) {
								$start_session = ("UPDATE $db->table_sessions SET active=1, sid=('" . $_SESSION['ID'] . "'), current_user_ip=('" . $client_ip . "'), session_started=NOW(), persistent_session_status= ('" . $persistentSess . "') WHERE id=(SELECT id FROM $db->table_accounts WHERE username= ('" . $username_validated . "'))");
								$db->query($start_session);
								if (!$start_session) {
									$db->query("INSERT INTO $db->table_accountlogs (message_fail, account_id, sid, user_ip, user_agent) VALUES ('Query Error! Konnte Sitzungsdaten nicht aktualisieren!', (SELECT id FROM $db->table_accounts WHERE username= ('" . $username_validated . "')), ('" . $_SESSION['ID'] . "'), ('" . $client_ip . "'), ('" . $userAgent . "'))");
									$errorStatus = true;
									$criticalLoginError = true;
								} //!$start_session
								else {
									$db->query("INSERT INTO $db->table_accountlogs (message, account_id, sid, user_ip, user_agent) VALUES ('User erfolgreich eingeloggt', (SELECT id FROM $db->table_accounts WHERE username= ('" . $username_validated . "')), ('" . $_SESSION['ID'] . "'), ('" . $client_ip . "'), ('" . $userAgent . "'))");
									$errorStatus = false;
									$successStatus = true;
								}
								...
								if ($set_user_session) {
									...
									$errorStatus = false;
									$successStatus = true;
									$externalMsg = $langGlobal["login_success"];
								} //$set_user_session
								else {
									$db->query("INSERT INTO $db->table_accountlogs (message, account_id, sid, user_ip, user_agent) VALUES ('Query Error! Konnte Sitzungsdaten nicht aktualisieren!', (SELECT id FROM $db->table_accounts WHERE username= ('" . $username_validated . "')), ('" . $_SESSION['ID'] . "'), ('" . $client_ip . "'), ('" . $userAgent . "'))");
									$errorStatus = true;
									$criticalLoginError = true;
								}
							}
