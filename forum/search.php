...
$min_search_length = $main->serverConfig('min_search_length');
$max_search_length = $main->serverConfig('max_search_length');

$search_key = $_GET['q'];

if(strlen($search_key) > $max_search_length)
{
	ThrowError_cC('Ihre Suche überschreitet die maximale Zeichenlänge von '.$max_search_length.' Zeichen!');
}
if(strlen($search_key) < $min_search_length)
{
	ThrowError_cC('Ihre Suche unterschreitet die minimale Zeichenlänge von '.$min_search_length.' Zeichen!');
}

if(!(strlen($search_key) > $max_search_length) && !(strlen($search_key) < $min_search_length))
{
	...
	$such_query = $db->query($such_query."ORDER BY date_posted DESC");
		...
		<div class="search">
		  <form method="GET" action="?page=Search">
			...
			<input type="text" placeholder="Suchen ..." name="q" value="'.$_GET['q'].'" maxlength="'.$main->serverConfig('max_search_length').'">
